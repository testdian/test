...
export function SupplierPlanFormFields({
  ...
  return (
    <>
      <Form.Item label='目标值'>
        <Input value={targetValue} disabled />
      </Form.Item>
      ...
      <Form.Item
        ...
        label={
          <FormLabelWithNote label='本月是否减排' note={SUPPLIER_PLAN_FORM_NOTE} />
        }
      ...
      <Form.Item
        ...
        rules={
          readOnly
            ? undefined
            : [
                { required: true, message: '请输入当月实际排放量' },
                positiveNumberRule(),
              ]
        }
      >
      ...
      {isReducing && (
        <>
          ...
          >
            <Radio.Group
              options={PLAN_REDUCTION_CATEGORY_OPTIONS}
              disabled={readOnly}
            />
          </Form.Item>
            ...
            rules={
              readOnly
                ...
                : [
                    ...
                    { max: MAX_PLAN_MEASURES_LENGTH, message: '不超过1000个字符' },
                  ]
            }
          >
            <Input.TextArea
          ...
          <Form.Item
            name='monthly_reduction'
            label='当月减排量（tCO2e）'
            rules={
              readOnly
                ? undefined
                : [{ required: true, message: '请输入当月减排量' }]
            }
          >
            <InputNumber
              min={0}
