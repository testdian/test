...
export const TARGET_YEAR_NOTE = '目标年度，必选，下拉选项，年份下拉，2026-2099';
...
export const SUGGESTIONS_NOTE = '低碳改善建议，文本框，必输，不超过500个字符';

export const PRODUCT_NAME_NOTE = '产品名称，文本框，必输，不超过100个字符';
...
export function syncOrgCarbonTargets(
  org?: OrgCarbonTarget,
): OrgCarbonTarget | undefined {
  if (!org) return org;
  return {
    ...org,
...
export function buildTargetSummary(payload: {
  ...
  if (payload.categories?.includes('org') && payload.org_carbon) {
    const { scope1_target_emission, scope2_target_emission } =
      payload.org_carbon;
    parts.push(
      `组织碳：范围一${scope1_target_emission ?? '-'}tCO₂e，范围二${
        scope2_target_emission ?? '-'
      }tCO₂e`,
    );
  }
  if (payload.categories?.includes('product') && payload.product_carbon) {
    ...
    parts.push(
      `产品碳：${product_name || '-'} ${target_footprint ?? '-'}tCO₂e/${
        functional_unit || '功能单位'
      }`,
    );
  }
  return parts.join('；') || '-';
