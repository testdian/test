...
import { transformData } from '@/views/cbam/ReportForm/Info/components/IndustrialProcessFlowDiagram/until';
...
const IndustrialProcessFlowDiagram = (
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const { data } = await getProcessResultEdge({ authNo });
